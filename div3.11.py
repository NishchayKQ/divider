...
print_output(f"total is {format(float(total), ',.2f')}\n"
             f"per head its {format(float(perPersonMuns), ',.2f')}")
toPay = []
...
for ara in listOfPeeps:
    ...
    if temp > 0:
        print_output(f"\t{ara} has to pay {format(float(temp), ',.2f')}")
    elif temp < 0:
        print_output(f"\t{ara} has to receive "
                     f"{format(float(abs(temp)), ',.2f')}")
    else:
            ...
            while temp > 0:
                if (toPay[i][0] > 0) and (abs(toPay[i][0]) <= temp):
                    ...
                    print_output(f"\t{toPay[i][1]} pays INR "
                                 f"{format(float(toPay[i][0]), ',.2f')} to "
                                 f"{ara[1]}")
